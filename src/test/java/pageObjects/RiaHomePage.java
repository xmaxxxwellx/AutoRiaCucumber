...
public class RiaHomePage {
    ...
    public RiaResultPage getResultPageByCategory(String categoryName){
        // Start actions
        Actions builder = new Actions(getDriver());
        builder.click(categoriesDropDownList);
        for (WebElement item : categoriesItems) {
            if(categoryName.equals(item.getAttribute("innerHTML"))) {
                item.click();
                break;
            }
        }
    ...
    public RiaResultPage getResultPageByRegionAndPrice(String region, String priceFrom, String priceTo){
        regionsDropDownList.click();
        selectRegion(region);
